...
A finished Zap ships twice, and both templates here have. Each ships as a folder that a reader rebuilds step by step and can edit anywhere, and as a shared Zapier template link that imports the whole thing and prompts only for the parts that should change. The media monitor's link opens with the feed URLs, relevance threshold, digest time, and channel editable. The roll-up's opens with the Notion database id and the Slack target. Both lock the prompt, because the prompt is where the rules live and a guided import is not the place to rewrite them. A shared link carries no credentials either way: whoever imports it connects their own accounts.

The two routes are not equivalent, and the README says which one a given change needs. Retargeting the monitor at a different beat means editing the prompt and its five output fields, so it is a rebuild from the folder, not a guided import.
